...
from constants import SIDE_SHORT, DATE_FORMAT
...
def find_roll_candidates(symbol, expiry, strike, premium, option_type='put'):
  ...
  print(f"Current asking is ${current_ask}.")

  strike_mask = ds.df['strike'] <= strike
  ...
  for expiry_date in ds.expiry_dates[:4]:
    ...
    mask = expiry_mask
    expiry_chain = dsdf[mask][::-1]  # reverse to show in chronological order
    ...
    for index, row in expiry_chain.iterrows():
      ...
      if agg < -1:
        continue
      print(f"{desc}: bid=${bid}, net=${net:.2f}, agg=${agg:.2f}")
...
if __name__ == '__main__':
  ...
  find_roll_candidates(symbol, expiry, strike, premium, option_type)
